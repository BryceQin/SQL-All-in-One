...
const spark_keywords_1 = require("./spark.keywords");
const spark_functions_1 = require("./spark.functions");
...
exports.spark = {
    ...
    tokenizerOptions: {
        ...
        supportsXor: true,
        reservedKeywords: spark_keywords_1.keywords,
        reservedDataTypes: spark_keywords_1.dataTypes,
        reservedFunctionNames: spark_functions_1.functions,
        extraParens: ["[]"],
        stringTypes: [
            "''-bs",
            '""-bs',
...
function postProcess(tokens) {
    return tokens.map((token, i) => {
        ...
        // [WINDOW](...)
        if (token_1.isToken.WINDOW(token) && nextToken.type === token_1.TokenType.OPEN_PAREN) {
        ...
        if (token.text === "ITEMS" &&
            token.type === token_1.TokenType.RESERVED_KEYWORD) {
            if (!(prevToken.text === "COLLECTION" &&
                nextToken.text === "TERMINATED")) {
