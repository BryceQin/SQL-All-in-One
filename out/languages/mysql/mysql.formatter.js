...
const mysql_keywords_1 = require("./mysql.keywords");
const mysql_functions_1 = require("./mysql.functions");
const token_1 = require("../../lexer/token");
// Shared functionality used by all MariaDB-like SQL dialects.
function postProcess(tokens) {
    return tokens.map((token, i) => {
        const nextToken = tokens[i + 1] || token_1.EOF_TOKEN;
        if (token_1.isToken.SET(token) && nextToken.text === "(") {
            // This is SET datatype, not SET statement
            return { ...token, type: token_1.TokenType.RESERVED_FUNCTION_NAME };
        }
        const prevToken = tokens[i - 1] || token_1.EOF_TOKEN;
        if (token_1.isToken.VALUES(token) && prevToken.text === "=") {
...
const reservedSelect = (0, expandPhrases_1.expandPhrases)(["SELECT [ALL | DISTINCT | DISTINCTROW]"]);
...
const reservedSetOperations = (0, expandPhrases_1.expandPhrases)(["UNION [ALL | DISTINCT]"]);
...
exports.mysql = {
    ...
    tokenizerOptions: {
        ...
        supportsXor: true,
        reservedKeywords: mysql_keywords_1.keywords,
        reservedDataTypes: mysql_keywords_1.dataTypes,
        reservedFunctionNames: mysql_functions_1.functions,
