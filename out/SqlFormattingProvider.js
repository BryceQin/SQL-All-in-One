"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.SqlFormattingProvider = void 0;
const vscode = __importStar(require("vscode"));
const config_1 = require("./config");
const formatEditorText_1 = require("./formatEditorText");
class SqlFormattingProvider {
    language;
    constructor(language) {
        this.language = language;
    }
    provideDocumentFormattingEdits(document, formattingOptions) {
        try {
            return [
                vscode.TextEdit.replace(this.fullDocumentRange(document), this.formatText(this.getAllText(document), formattingOptions, document.uri)),
            ];
        }
        catch (e) {
            vscode.window.showErrorMessage("Unable to format SQL:\n" + e);
            return [];
        }
    }
    getAllText(document) {
        // extract all lines from document
        return [...new Array(document.lineCount)]
            .map((_, i) => document.lineAt(i).text)
            .join("\n");
    }
    fullDocumentRange(document) {
        return new vscode.Range(document.positionAt(0), document.lineAt(document.lineCount - 1).range.end);
    }
    formatText(text, formattingOptions, uri) {
        const extensionSettings = vscode.workspace.getConfiguration("Hive-Formatter", uri);
        const formatConfig = (0, config_1.createConfig)(extensionSettings, formattingOptions, this.language);
        return (0, formatEditorText_1.formatEditorText)(text, formatConfig);
    }
}
exports.SqlFormattingProvider = SqlFormattingProvider;
//# sourceMappingURL=SqlFormattingProvider.js.map