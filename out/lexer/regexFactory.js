 ...
 */
function lineComment(lineCommentTypes) {
    return new RegExp(`(?:${lineCommentTypes
        .map(regexUtil_1.escapeRegExp)
        .join("|")}).*?(?=\r\n|\r|\n|$)`, "uy");
}
/**
 ...
 */
function parenthesis(kind, extraParens = []) {
    let index;
    if (kind === "open") {
        index = 0;
    }
    ...
    return (0, regexUtil_1.patternToRegex)(parens.map(regexUtil_1.escapeRegExp).join("|"));
}
/**
...
function operator(operators) {
    return (0, regexUtil_1.patternToRegex)(`${(0, utils_1.sortByLengthDesc)(operators).map(regexUtil_1.escapeRegExp).join("|")}`);
}
...
function rejectIdentCharsPattern({ rest, dashes }) {
 ...
 */
function reservedWord(reservedKeywords, identChars = {}) {
    if (reservedKeywords.length === 0) {
        return /^\b$/u;
    }
    const avoidIdentChars = rejectIdentCharsPattern(identChars);
    const reservedKeywordsPattern = (0, utils_1.sortByLengthDesc)(reservedKeywords)
        .map(regexUtil_1.escapeRegExp)
        .join("|")
        .replace(/ /gu, "\\s+");
 ...
 */
function parameter(paramTypes, pattern) {
    // 1. 边界判断：无占位符类型时，返回undefined（无需构建正则）
    if (!paramTypes.length) {
        return undefined;
    }
    // 2. 处理占位符类型：转义 + 拼接成“或”分支
    const typesRegex = paramTypes.map(regexUtil_1.escapeRegExp).join("|");
    // 3. 拼接类型 + 主体模式，转成最终正则
    return (0, regexUtil_1.patternToRegex)(`(?:${typesRegex})(?:${pattern})`);
}
/**
 * 构建匹配 PostgreSQL 风格「Q 字符串（Quoted String）」的正则表达式模板
 */
function buildQStringPatterns() {
    ...
    const patternList = Object.entries(specialDelimiterMap).map(([left, right]) => singlePattern
        .replace(/{left}/g, (0, regexUtil_1.escapeRegExp)(left))
        .replace(/{right}/g, (0, regexUtil_1.escapeRegExp)(right)));
    // 4. 生成「特殊分隔符左字符」的转义字符串（用于排除标准分隔符）
    const specialDelimiters = (0, regexUtil_1.escapeRegExp)(Object.keys(specialDelimiterMap).join(""));
...
function singleQuotePattern(quoteTypes) {
    if (typeof quoteTypes === "string") {
        ...
        return (0, regexUtil_1.prefixesPattern)(quoteTypes) + exports.quotePatterns[quoteTypes.quote];
    }
}
// 「变量 Token」。将数组中每个变量规则转换为对应的正则片段，拼接成 “或” 分支后编译为最终的 RegExp 对象
function variable(varTypes) {
    return (0, regexUtil_1.patternToRegex)(varTypes
        .map((varType) => "regex" in varType
        ? varType.regex
        : singleQuotePattern(varType))
        .join("|"));
}
// 生成匹配所有指定引号类型的正则字符串（引号分隔模式）
function stringPattern(quoteTypes) {
...
function string(quoteTypes) {
    return (0, regexUtil_1.patternToRegex)(stringPattern(quoteTypes));
}
// 基于 IdentChars 配置（自定义首字符、后续字符、中划线、数字开头等规则），结合 Unicode 字符支持，动态生成适配特定 SQL 方言的标识符匹配正则字符串，是 identifier 函数的底层核心逻辑
function identifierPattern({ first, rest, dashes, allowFirstCharNumber, } = {}) {
    // 1. 基础字符定义（Unicode 兼容）
    ...
    const firstChars = (0, regexUtil_1.escapeRegExp)(first ?? "");
    const restChars = (0, regexUtil_1.escapeRegExp)(rest ?? "");
    ...
    return dashes ? (0, regexUtil_1.withDashes)(pattern) : pattern;
}
//「标识符 Token」。将「业务层面的标识符字符规则（IdentChars）」，通过 identifierPattern 转换为正则字符串，再经统一的 patternToRegex 编译为可执行的 RegExp
function identifier(specialChars = {}) {
    return (0, regexUtil_1.patternToRegex)(identifierPattern(specialChars));
}
