...
function registerDiagnostics(context) {
    ...
    const getDp = () => {
        if (!dp) {
            dp = container.get(diContainer_1.Tokens.SqlDiagnosticsProvider);
            if (dp)
...
function registerProviders(context) {
    ...
    let renameProvider;
    const lazyCodeAction = () => codeActionProvider ??= container.get(diContainer_1.Tokens.CodeActionProvider);
    const lazyFoldingRange = () => foldingRangeProvider ??= container.get(diContainer_1.Tokens.FoldingRangeProvider);
    const lazyOutline = () => outlineProvider ??= container.get(diContainer_1.Tokens.OutlineProvider);
    const lazyHover = () => hoverProvider ??= container.get(diContainer_1.Tokens.HoverProvider);
    const lazyDefinition = () => definitionProvider ??= container.get(diContainer_1.Tokens.DefinitionProvider);
    const lazyReference = () => referenceProvider ??= container.get(diContainer_1.Tokens.ReferenceProvider);
    const lazyRename = () => renameProvider ??= container.get(diContainer_1.Tokens.RenameProvider);
...
function registerCompletion(context) {
    ...
    const getProvider = () => {
        if (!provider) {
            provider = container.get(diContainer_1.Tokens.CompletionProvider);
            if (provider)
...
function registerParameterHighlighter(context) {
    ...
    const getHighlighter = () => {
        if (!highlighter) {
            highlighter = container.get(diContainer_1.Tokens.ParameterHighlighter);
            if (highlighter)
...
function registerAstNavigatorEvents(context) {
    const container = (0, diContainer_1.getContainer)();
    let navigator;
    const getNavigator = () => navigator ??= container.get(diContainer_1.Tokens.AstNavigator);
...
function registerStatusBar(context) {
    ...
    const getStatusBar = () => {
        if (!statusBar) {
            statusBar = container.get(diContainer_1.Tokens.StatusBarProvider);
            if (statusBar)
