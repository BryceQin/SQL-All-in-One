...
const IMPLEMENTED_DIALECTS = ["mysql", "postgresql", "sqlite"];
...
function init() {
    const config = window.__CONNECTION_DIALOG_CONFIG__ || {};
...
function renderDialectGrid() {
    ...
    Object.keys(DIALECT_INFO).forEach(function (key) {
        const info = DIALECT_INFO[key];
        const card = document.createElement("div");
        const isActive = state.dialect === key;
        const isImplemented = info.implemented !== false;
        card.className = "dialect-card" + (isActive ? " active" : "") + (!isImplemented ? " disabled" : "");
        card.setAttribute("data-dialect", key);
        card.innerHTML =
            '<div class="dialect-card-icon">' +
            info.icon +
            '</div><div class="dialect-card-name">' +
            info.name +
            (!isImplemented ? ' <small style="opacity:0.5">(Soon)</small>' : "") +
            "</div>";
        if (isImplemented) {
...
function switchTab(tabName) {
    ...
    const tabBtn = document.querySelector('.conn-form-tab[data-tab="' + tabName + '"]');
    if (tabBtn) {
...
function updateDialectUI() {
    const isSqlite = state.dialect === "sqlite";
...
function updateSshAuthMethodUI() {
    const method = state.ssh.authentication;
...
function renderGroupDropdown() {
    ...
    const currentValue = state.group;
...
function showTestResult(type, message) {
    ...
    if (type === "loading") {
        ...
        result.innerHTML =
            '<svg class="conn-form-result-icon" width="14" height="14" viewBox="0 0 16 16"><path d="M3 8l3 3 7-7" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg><span class="conn-form-result-text"></span>';
        result.querySelector(".conn-form-result-text").textContent = message;
    } else {
        result.innerHTML =
            '<svg class="conn-form-result-icon" width="14" height="14" viewBox="0 0 16 16"><circle cx="8" cy="8" r="7" stroke="currentColor" stroke-width="1.3" fill="none"/><path d="M8 4v5M8 11v1" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg><span class="conn-form-result-text"></span>';
        result.querySelector(".conn-form-result-text").textContent = message;
    }
}
    ...
    switch (message.command) {
        ...
        case "fileSelected":
            if (message.field && message.path) {
                let fieldId = message.field;
                if (fieldId === "sqliteFile") {
                    fieldId = "connSqliteFile";
                }
