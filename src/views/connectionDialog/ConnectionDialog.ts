...
import type { IConnectionService, IConnectionStore, IDialectMetadataProvider } from "../../application/ports";
...
export class ConnectionDialog extends BaseWebviewPanel {
    ...
    private async _buildConfig(options: { mode: "create" | "edit"; connectionId?: string }): Promise<ConnectionDialogConfig> {
        ...
        const existingNames = connections.filter((c) => c.id !== options.connectionId).map((c) => c.name);
