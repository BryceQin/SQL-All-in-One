...
function renderVisibleRows() {
    ...
    for (var i = startRow; i < endRow; i++) {
        ...
        var rowChange = isPlaceholder
            ? null
            : state.pendingChanges.find(function (c) {
                  return c.rowIndex === i;
              });
        if (rowChange) {
...
function trackChange(row, col, oldVal, newVal) {
    var existing = state.pendingChanges.find(function (c) {
...
function deleteRow() {
    ...
    var existingInsert = state.pendingChanges.find(function (c) {
        return c.rowIndex === row && c.type === "insert";
    });
    if (existingInsert) {
        state.pendingChanges = state.pendingChanges.filter(function (c) {
            return c !== existingInsert;
        });
        ...
        var existingDelete = state.pendingChanges.find(function (c) {
            return c.rowIndex === row && c.type === "delete";
        });
        if (existingDelete) {
            state.pendingChanges = state.pendingChanges.filter(function (c) {
                return c !== existingDelete;
            });
        } else {
            var existingUpdate = state.pendingChanges.find(function (c) {
                return c.rowIndex === row && c.type === "update";
            });
            if (existingUpdate) {
                state.pendingChanges = state.pendingChanges.filter(function (c) {
                    return c !== existingUpdate;
                });
            }
            var primaryKey = getPrimaryKeyValue(row);
            state.pendingChanges.push({
...
function commitChanges() {
    ...
    var updateCount = state.pendingChanges.filter(function (c) {
        return c.type === "update";
    }).length;
    var insertCount = state.pendingChanges.filter(function (c) {
        return c.type === "insert";
    }).length;
    var deleteCount = state.pendingChanges.filter(function (c) {
