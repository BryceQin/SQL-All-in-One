...
function removeBlockComment(editor: vscode.TextEditor, selection: vscode.Selection, text: string): void {
    ...
    void vscode.workspace.applyEdit(edit).then((success) => {
        if (success)
            getContainer().get<StatusBarProvider>(Tokens.StatusBarProvider)?.showTemporaryMessage(t("notification.commentRemoved"));
    });
}
...
function addColumnComment(editor: vscode.TextEditor): void {
    ...
    const insertPos = hasComma ? line.range.end.translate(0, -1) : line.range.end;
    ...
    void vscode.workspace.applyEdit(edit).then((success) => {
        if (success) {
            ...
            if (quoteStart > 8 && quoteEnd !== -1) {
                editor.selection = new vscode.Selection(editor.selection.active.line, quoteStart, editor.selection.active.line, quoteEnd);
            }
...
function wrapWithFormatterDisable(editor: vscode.TextEditor): void {
    ...
    void vscode.workspace.applyEdit(edit).then((success) => {
        if (success)
            getContainer()
                .get<StatusBarProvider>(Tokens.StatusBarProvider)
                ?.showTemporaryMessage(t("notification.formatDisabledMarkAdded"));
    });
}
