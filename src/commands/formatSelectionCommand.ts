...
function replaceEachSelection(editor: vscode.TextEditor, fn: (code: string) => string): void {
...
const createConfigForEditor = (editor: vscode.TextEditor): FormatOptionsWithLanguage =>
    createConfig(vscode.workspace.getConfiguration("SQL-All-in-One"), editorFormattingOptions(editor), detectSqlDialect(editor));

const detectSqlDialect = (editor: vscode.TextEditor): SqlLanguage => sqlDialects[editor.document.languageId] ?? "sql";
