    ...
    private async _updateConfig(data: Record<string, unknown>): Promise<void> {
        ...
        for (const item of ALL_CONFIG_ITEMS) {
            ...
            try {
                ...
                /* skip: best-effort config update */ console.debug("[SQL All in One] ConfigEditor config update skipped:", configKey, e);
            }
        }
        ...
        for (const rule of LINT_RULES) {
            ...
            try {
                ...
                /* skip: best-effort lint rule update */ console.debug(
                    "[SQL All in One] ConfigEditor lint rule update skipped:",
                    rule.configKey,
                    e,
                );
            }
