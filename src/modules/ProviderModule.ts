...
export class ProviderModule implements Activatable {
    ...
    private registerAll(context: vscode.ExtensionContext): void {
        ...
        const lazyCompletionProvider: vscode.CompletionItemProvider = {
            provideCompletionItems: (doc, pos, token, _ctx) => getCompletionProvider()?.provideCompletionItems(doc, pos, token),
            resolveCompletionItem: (item, token) => getCompletionProvider()?.resolveCompletionItem(item, token) ?? item,
        };
