...
export class SqlHoverProvider implements vscode.HoverProvider {
    ...
    constructor() {
        this.docResolvers = [new ParameterHoverResolver(), new FunctionHoverResolver()];
        this.schemaResolver = new SchemaHoverResolver();
        this.staticResolvers = [new KeywordHoverResolver()];
    }
        ...
        return getPerformanceMonitor().measureAsync("SqlHoverProvider.provideHover", async () => {
            ...
            if (activeConn) {
                try {
                    ...
                    /* schema hover failed, fallback */ handleError(e, "SqlHoverProvider.provideHover", ErrorCategory.SUB_ITEM);
                }
            }
