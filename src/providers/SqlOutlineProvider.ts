...
export class SqlOutlineProvider implements vscode.DocumentSymbolProvider {
    ...
    private provideDocumentSymbolsFromAst(document: vscode.TextDocument, ast: unknown[] | unknown): vscode.DocumentSymbol[] {
    ...
    private processAstStatement(document: vscode.TextDocument, node: AstNode): vscode.DocumentSymbol | null {
    ...
    private processSelectStatement(document: vscode.TextDocument, node: AstNode): vscode.DocumentSymbol | null {
    ...
    private processSelectChain(document: vscode.TextDocument, node: AstNode, siblings: vscode.DocumentSymbol[]): void {
    ...
    private processWithClause(document: vscode.TextDocument, withClause: unknown): vscode.DocumentSymbol | null {
        ...
        for (const item of cteItems) {
            ...
            if (cteName && isAstNode(item)) {
                const cteSymbol = this.createSymbolFromAst(document, cteName, "CTE", vscode.SymbolKind.Constant, item as AstNode);
                if (cteSymbol) {
        ...
        const endPos = endLoc ? new vscode.Position(endLoc.line - 1, endLoc.column - 1) : startPos;

        const range = new vscode.Range(startPos, endPos);
        const withSymbol = new vscode.DocumentSymbol(t("outline.withClause"), t("outline.cte"), vscode.SymbolKind.Namespace, range, range);
        withSymbol.children = cteSymbols;

        return withSymbol;
    }

    private processDmlStatement(document: vscode.TextDocument, node: AstNode, dmlType: string): vscode.DocumentSymbol | null {
        const tableName = dmlType === "DELETE" ? this.extractTableNameFromFrom(node) : this.extractTableNameFromTable(node);
    ...
    private processCreateStatement(document: vscode.TextDocument, node: AstNode): vscode.DocumentSymbol | null {
    ...
    private processSelectColumns(_document: vscode.TextDocument, columns: unknown[]): vscode.DocumentSymbol | null {
            ...
            if (colEntry.as) {
                ...
                if (aliasName) {
                    ...
                    if (loc?.start?.line && loc?.start?.column) {
                        ...
                        const endPos =
                            loc?.end?.line && loc?.end?.column ? new vscode.Position(loc.end.line - 1, loc.end.column - 1) : startPos;
                        const range = new vscode.Range(startPos, endPos);
        ...
        const endPos = lastLoc?.end ? new vscode.Position(lastLoc.end.line - 1, lastLoc.end.column - 1) : startPos;
        const range = new vscode.Range(startPos, endPos);
    ...
    private processFromClauses(_document: vscode.TextDocument, from: unknown[]): vscode.DocumentSymbol[] {
        const symbols: vscode.DocumentSymbol[] = [];
        ...
        for (const item of from) {
            ...
            if (typeof join === "string") {
                ...
                label = alias ? `${joinUpper} ${tableName || "..."} ${alias}` : `${joinUpper} ${tableName || "..."}`;
            } else {
                label = alias ? `FROM ${tableName || "..."} ${alias}` : `FROM ${tableName || "..."}`;
            }
    ...
    private createClauseSymbol(document: vscode.TextDocument, clause: unknown, clauseName: string): vscode.DocumentSymbol | null {
        ...
        return new vscode.DocumentSymbol(
            label,
            "",
            clauseName === "WHERE" || clauseName === "HAVING" ? vscode.SymbolKind.Boolean : vscode.SymbolKind.Array,
            range,
            range,
        );
    }

    private createClauseSymbolFromArray(document: vscode.TextDocument, items: unknown, clauseName: string): vscode.DocumentSymbol | null {
