...
export class SqlDiagnosticsProvider {
    ...
    constructor(astDiagnosticsProvider: AstDiagnosticsProvider, linter: SqlLinter) {
        this.diagnosticCollection = vscode.languages.createDiagnosticCollection("hive-formatter");
        this.astDiagnosticsProvider = astDiagnosticsProvider;
        this.linter = linter;
        ...
        this.configChangeDisposable = getConfigManager().onConfigChange(() => {
            ...
            if (configManager.checkLinterConfigChanged()) {
                ...
                const visibleUris = new Set(
                    vscode.window.visibleTextEditors.filter((ed) => isSqlDocument(ed.document)).map((ed) => ed.document.uri.toString()),
                );
    ...
    public async provideDiagnostics(document: vscode.TextDocument, token?: vscode.CancellationToken): Promise<void> {
        await getPerformanceMonitor().measureAsync("SqlDiagnosticsProvider.provideDiagnostics", async () => {
            ...
            try {
                ...
                const astList =
                    parseResult.success && parseResult.ast ? (Array.isArray(parseResult.ast) ? parseResult.ast : [parseResult.ast]) : [];
    ...
    private createDiagnosticFromError(error: unknown, text: string, document: vscode.TextDocument): vscode.Diagnostic | undefined {
