...
export function lineComment(lineCommentTypes: string[]): RegExp {
    return new RegExp(`(?:${lineCommentTypes.map(escapeRegExp).join("|")}).*?(?=\r\n|\r|\n|$)`, "uy");
}
/**
 ...
 */
export function parenthesis(kind: "open" | "close", extraParens: ("[]" | "{}")[] = []): RegExp {
    let index: number;
    if (kind === "open") {
        index = 0;
    } else {
...
export function operator(operators: string[]): RegExp {
    return patternToRegex(`${sortByLengthDesc(operators).map(escapeRegExp).join("|")}`);
}
 ...
 */
export function reservedWord(reservedKeywords: string[], identChars: IdentChars = {}): RegExp {
 ...
 */
export function parameter(paramTypes: string[], pattern: string): RegExp | undefined {
...
export function variable(varTypes: VariableType[]): RegExp {
    return patternToRegex(varTypes.map((varType) => ("regex" in varType ? varType.regex : singleQuotePattern(varType))).join("|"));
}
...
export function identifierPattern({ first, rest, dashes, allowFirstCharNumber }: IdentChars = {}): string {
    // 1. 基础字符定义（Unicode 兼容）
