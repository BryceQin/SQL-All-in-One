...
import { isReserved, TokenType } from "./token"
...
const propertyNameKeywordToIdent = (
    token: Token,
    i: number,
    tokens: Token[],
): Token => {
    ...
    if (isReserved(token.type)) {
        ...
        if (
            prevToken &&
            prevToken.type === TokenType.PROPERTY_ACCESS_OPERATOR
        ) {
        ...
        if (
            nextToken &&
            nextToken.type === TokenType.PROPERTY_ACCESS_OPERATOR
        ) {
            return { ...token, type: TokenType.IDENTIFIER, text: token.raw }
        }
    }
...
const dataTypeToParameterizedDataType = (
    token: Token,
    i: number,
    tokens: Token[],
): Token => {
    if (token.type === TokenType.RESERVED_DATA_TYPE) {
        ...
        if (nextToken && isOpenParen(nextToken)) {
            return {
                ...token,
                type: TokenType.RESERVED_PARAMETERIZED_DATA_TYPE,
            }
        }
    }
    return token
...
const prevNonCommentToken = (
    tokens: Token[],
    index: number,
): Token | undefined => nextNonCommentToken(tokens, index, -1)
...
// 判断是否是左括号 (
const isOpenParen = (t: Token): boolean =>
    t.type === TokenType.OPEN_PAREN && t.text === "("

// 判断是否是左中括号 [
const isOpenBracket = (t: Token): boolean =>
    t.type === TokenType.OPEN_PAREN && t.text === "["
