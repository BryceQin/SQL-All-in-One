...
export class MissingPrimaryKeyRule extends BaseRule {
    ...
    readonly name = "linter.createTableWithoutPK.name";
    ...
    check(context: RuleContext): vscode.Diagnostic[] {
        ...
        for (const def of createDefinitions) {
            ...
            if (
                defNode.primary_key === true ||
                defNode.primary_key === "primary key" ||
                defNode.primary_key === "key" ||
                defNode.primary === "key" ||
                defNode.primary === "primary key"
            ) {
                hasPrimaryKey = true;
                break;
            }
            ...
            if (Array.isArray(definition)) {
                for (const item of definition) {
                    if (isAstNode(item)) {
                        ...
                        if (
                            itemNode.type === "column_ref" &&
                            typeof itemNode.column === "string" &&
                            itemNode.column.toLowerCase() === "primary"
                        ) {
                            hasPrimaryKey = true;
                            break;
                        }
