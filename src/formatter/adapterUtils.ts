...
export function extractUntilNextClause(text: string): string {
    const clauseKeywords =
        /\b(?:WHERE|GROUP\s+BY|HAVING|ORDER\s+BY|SORT\s+BY|CLUSTER\s+BY|DISTRIBUTE\s+BY|LIMIT|LATERAL\s+VIEW|JOIN|LEFT|RIGHT|INNER|OUTER|CROSS|FULL|UNION|INTERSECT|EXCEPT|SET|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP)\b/i;
