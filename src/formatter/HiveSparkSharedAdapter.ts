...
import { KeywordOccurrence, matchCase, extractUntilNextClause, collectOrderedMatches, cleanClusterByAscDesc } from "./adapterUtils";
...
const allOrderByPatterns = [{ pattern: /\bORDER\s+BY\b/gi, keyword: "ORDER BY", hive: "ORDER BY" }, ...orderByFamily];
const allGroupByPatterns = [{ pattern: /\bGROUP\s+BY\b/gi, keyword: "GROUP BY", hive: "GROUP BY" }, ...groupByFamily];
...
function restoreKeywordGroup(sql: string, hiveKeyword: string, restorations: KeywordOccurrence[]): string {
...
const lateralViewPatterns: RegExp[] = [/\bLATERAL\s+VIEW\s+OUTER\b/gi, /\bLATERAL\s+VIEW\b/gi];
...
export function extractLateralView(
    ...
    for (const pattern of lateralViewPatterns) {
        ...
        for (let i = matches.length - 1; i >= 0; i--) {
            ...
            result = result.substring(0, lvMatch.index) + `CROSS JOIN ${id}` + result.substring(lvMatch.index + lvMatch.text.length);
        }
    }
...
export function restoreLateralView(formatted: string, slots: LateralViewSlotBase[], idMarker: string): string {
    let result = formatted;
...
export { replaceSortDistributeCluster, restoreSortDistributeCluster, extractUntilNextClause, escapeRegExp };
