...
const MYSQL_TYPE_REGEXES: [RegExp, string][] = Object.entries(MYSQL_TO_HIVE_TYPES).map(([mysqlType, hiveType]) => [
    new RegExp(`\\b${mysqlType}\\b`, "gi"),
    hiveType,
]);
...
export class MysqlToHiveConverter {
    ...
    private removeTypeLength(item: string): string {
        return item.replace(
            /\b(STRING|VARCHAR|CHAR|TINYINT|SMALLINT|INT|INTEGER|BIGINT)\s*\(\s*\d+\s*\)/gi,
            (_match: string, type: string): string => {
