...
function findColumnAliasRefsInExpr(
    expr: unknown,
    nameLower: string,
    context: string,
    document: vscode.TextDocument,
...
function findReferencesInSelect(
    node: AstNode,
    symbolName: string,
    document: vscode.TextDocument,
...
export class AstNavigator {
    ...
    findReferences(ast: unknown[] | unknown, symbolName: string, document: vscode.TextDocument, symbolType: SymbolType): SymbolReference[] {
    ...
    getDefinition(word: string, index: SymbolIndex): vscode.Location | null {
        const nameLower = word.toLowerCase();
        return (
            index.cteDefinitions.get(nameLower) ||
    ...
    hasDefinition(word: string, index: SymbolIndex): boolean {
        const nameLower = word.toLowerCase();
        return (
            index.cteDefinitions.has(nameLower) || index.tableAliasDefinitions.has(nameLower) || index.columnAliasDefinitions.has(nameLower)
        );
    }
}
