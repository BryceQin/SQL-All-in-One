...
export class SshTunnel {
    ...
    async open(sshConfig: SshConfig, targetHost: string, targetPort: number, timeout?: number): Promise<TunnelResult> {
        if (this._isOpen) {
    ...
    private async validateKeyPath(keyPath: string): Promise<string> {
        ...
        const allowedDirs = [homeDir, path.join(homeDir, ".ssh"), "/etc/ssh"];
        ...
        if (!isAllowed) {
            throw new Error(
                `SSH private key path not allowed: ${keyPath}. Key must be located in your home directory, .ssh folder, or /etc/ssh.`,
            );
        }
        return realPath;
    }
