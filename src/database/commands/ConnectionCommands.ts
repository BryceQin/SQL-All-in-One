...
export function registerConnectionCommands(context: vscode.ExtensionContext, _dbModule: DatabaseModule): vscode.Disposable[] {
        ...
        vscode.commands.registerCommand("hive-formatter.connect", async (node?: ITreeNode) => {
            if (node) {
                try {
                    ...
                    const shortMessage = fullError.length > 80 ? fullError.substring(0, 80) + "..." : fullError;
    ...
    disposables.push(
        vscode.commands.registerCommand("hive-formatter.exportConnections", async () => {
            ...
            try {
                ...
                const includePasswords = await vscode.window.showQuickPick([t("database.withoutPasswords"), t("database.withPasswords")], {
                    placeHolder: t("database.exportOptions"),
                });
                if (!includePasswords) return;
                await store.exportConnections(uri.fsPath, includePasswords === t("database.withPasswords"));
    ...
    disposables.push(
        vscode.commands.registerCommand("hive-formatter.importConnections", async () => {
            ...
            try {
                ...
                vscode.window.showInformationMessage(t("database.importedConnections", String(result.added), String(result.skipped)));
                vscode.commands.executeCommand("hive-formatter.refreshTreeProvider");
            } catch (error) {
