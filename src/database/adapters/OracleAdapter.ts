...
import { OracleSharedContext } from './OracleSharedContext';
import { OracleConnectionAdapter } from './OracleConnectionAdapter';
import { OracleQueryAdapter } from './OracleQueryAdapter';
import { OracleMetadataAdapter } from './OracleMetadataAdapter';
import { OracleSchemaAdapter } from './OracleSchemaAdapter';

/**
 * Oracle database adapter.
 *
 * Assembles the five Oracle sub-adapters (connection, query, metadata, schema,
 * shared context) and delegates the IDatabaseAdapter surface to them. The
 * oracledb driver (6.x, thin mode by default) is loaded lazily via dynamic
...
export class OracleAdapter extends BaseDatabaseAdapter {
    ...
    private connectionAdapter: OracleConnectionAdapter;
    private queryAdapter: OracleQueryAdapter;
    private metadataAdapter: OracleMetadataAdapter;
    private schemaAdapter: OracleSchemaAdapter;

    constructor(config: ConnectionConfig) {
        super(config);
        this.shared = new OracleSharedContext(this);
        this.connectionAdapter = new OracleConnectionAdapter(this.shared);
        this.queryAdapter = new OracleQueryAdapter(this.shared);
        this.metadataAdapter = new OracleMetadataAdapter(
            this.shared,
            (sql, params) => this.queryAdapter.execute(sql, params)
        );
        this.schemaAdapter = new OracleSchemaAdapter(
            this.shared,
            (sql, params) => this.queryAdapter.execute(sql, params),
        ...
        if (now - this.lastActivityTime > idleTimeout) {
            ...
            if (status.activeConnections === 0 && status.idleConnections > 0) {
                try {
                    ...
                    console.debug('[SQL All in One] Oracle reap idle connections error:', e);
                }
            }
        }
    }
