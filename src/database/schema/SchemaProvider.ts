...
export class SchemaProvider {
    ...
    static resetInstance(): void {
        if (SchemaProvider.instance) {
            SchemaProvider.instance.dispose()
        }
        SchemaProvider.instance = undefined as unknown as SchemaProvider
    }
    ...
    private async addViewItems(items: vscode.CompletionItem[], context: CompletionContext): Promise<void> {
        try {
            const adapter = ConnectionManager.getInstance().getAdapter(context.connectionId)
            if (!adapter) return
    ...
    async getTableHoverInfo(tableName: string, database: string): Promise<vscode.MarkdownString | null> {
        ...
        const adapter = ConnectionManager.getInstance().getAdapter(activeConn.id)
