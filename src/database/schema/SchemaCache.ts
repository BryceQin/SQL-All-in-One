...
export class SchemaCache {
    ...
    async getDatabases(connectionId: string): Promise<DatabaseInfo[]> {
        ...
        const request = (async () => {
            try {
                const adapter = ConnectionManager.getInstance().getAdapter(connectionId)
                if (!adapter) return []
    ...
    static getInstance(): SchemaCache {
        if (!SchemaCache.instance) {
            ...
            if (container.hasInstance(Tokens.SchemaCache)) {
                SchemaCache.instance = container.get<SchemaCache>(Tokens.SchemaCache);
            } else {
                SchemaCache.instance = new SchemaCache();
                container.register(Tokens.SchemaCache, SchemaCache.instance);
            }
        }
    ...
    static resetInstance(): void {
        if (SchemaCache.instance) {
            SchemaCache.instance.dispose()
        }
        SchemaCache.instance = undefined as unknown as SchemaCache
    }
