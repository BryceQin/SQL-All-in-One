...
suite("Table Designer - Validation", () => {
    ...
    test("should fail validation for duplicate column names", () => {
        const data = makeDesign({
            columns: [makeColumn({ name: "id" }), makeColumn({ name: "id" })],
        });
    ...
    test("should fail validation for duplicate column names (case insensitive)", () => {
        const data = makeDesign({
            columns: [makeColumn({ name: "Name" }), makeColumn({ name: "name" })],
        });
        const result = validateDesign(data);
        assert.ok(result);
        assert.ok(result.includes("Duplicate column names"));
...
suite("Table Designer - CREATE TABLE DDL", () => {
    ...
    test("should generate nullable column", () => {
        const data: TableDesignData = {
            tableName: "test",
            columns: [
                makeColumn({ name: "email", type: "VARCHAR", length: "255", nullable: true, isPrimaryKey: false, isAutoIncrement: false }),
            ],
            indexes: [],
            foreignKeys: [],
            triggers: [],
...
suite("Table Designer - CREATE TABLE with Foreign Keys", () => {
    test("should generate foreign key constraint", () => {
        const data: TableDesignData = {
            tableName: "orders",
            columns: [
                makeColumn({ name: "user_id", type: "INT", length: "11", isPrimaryKey: false, isAutoIncrement: false, nullable: false }),
            ],
            indexes: [],
            foreignKeys: [
                {
    ...
    test("should generate foreign key with SET NULL action", () => {
        const data: TableDesignData = {
            tableName: "orders",
            columns: [
                makeColumn({ name: "user_id", type: "INT", length: "11", isPrimaryKey: false, isAutoIncrement: false, nullable: true }),
            ],
            indexes: [],
            foreignKeys: [
                {
...
suite("Table Designer - ALTER TABLE DDL", () => {
    const originalStructure: TableStructure = {
        ...
        indexes: [{ name: "idx_name", type: "BTREE", columns: ["name"], isUnique: false, isPrimary: false }],
        foreignKeys: [],
        triggers: [],
        engine: "InnoDB",
        charset: "utf8mb4",
...
suite("Table Designer - ALTER TABLE Indexes", () => {
    ...
    test("should not generate DROP INDEX for primary key", () => {
        const originalWithPk: TableStructure = {
            columns: [{ name: "id", type: "INT", length: 11, nullable: false, isPrimaryKey: true, isAutoIncrement: true, isUnique: false }],
            indexes: [{ name: "PRIMARY", type: "BTREE", columns: ["id"], isUnique: true, isPrimary: true }],
            foreignKeys: [],
            triggers: [],
        };
        const data: TableDesignData = {
...
suite("Table Designer - ALTER TABLE Triggers", () => {
    const originalStructure: TableStructure = {
        ...
        indexes: [],
        foreignKeys: [],
        triggers: [{ name: "trg_before_insert", timing: "BEFORE", event: "INSERT", statement: "SET NEW.created_at = NOW()" }],
    };
...
suite("Table Designer - Complex Scenarios", () => {
    ...
    test("should generate multiple ALTER TABLE statements for mixed changes", () => {
        ...
        const data: TableDesignData = {
            ...
            indexes: [{ id: "idx1", name: "idx_email", type: "BTREE", columns: ["email"], isUnique: true }],
            foreignKeys: [],
            triggers: [],
            options: { engine: "InnoDB", charset: "utf8mb4", collation: "utf8mb4_general_ci", autoIncrement: "", comment: "new comment" },
            mode: "edit",
...
suite("Table Designer - Data Type Handling", () => {
    ...
    test("should generate DATETIME type", () => {
        const data: TableDesignData = {
            tableName: "test",
            columns: [
                makeColumn({ name: "created", type: "DATETIME", length: "", isPrimaryKey: false, isAutoIncrement: false, nullable: false }),
            ],
            indexes: [],
            foreignKeys: [],
            triggers: [],
    ...
    test("should generate JSON type", () => {
        const data: TableDesignData = {
            tableName: "test",
            columns: [
                makeColumn({ name: "metadata", type: "JSON", length: "", isPrimaryKey: false, isAutoIncrement: false, nullable: true }),
            ],
            indexes: [],
            foreignKeys: [],
            triggers: [],
