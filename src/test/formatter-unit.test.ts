...
import { expandSinglePhrase, expandPhrases } from "../dialects/expandPhrases";
...
suite("InlineLayout", () => {
    ...
    test("throws InlineLayoutError when content exceeds expressionWidth", () => {
        ...
        assert.throws(() => layout.add("SELECT"), InlineLayoutError);
    });

    test("throws InlineLayoutError on NEWLINE", () => {
        const layout = new InlineLayout(100);
        assert.throws(() => layout.add("SELECT", WS.NEWLINE, "FROM"), InlineLayoutError);
    });

    test("throws InlineLayoutError on MANDATORY_NEWLINE", () => {
        const layout = new InlineLayout(100);
        assert.throws(() => layout.add("SELECT", WS.MANDATORY_NEWLINE, "FROM"), InlineLayoutError);
    });
...
suite("expandPhrases", () => {
    ...
    test("expandSinglePhrase expands optional blocks [...]", () => {
        ...
        assert.deepStrictEqual(result, ["CREATE TABLE", "CREATE OR REPLACE TABLE"]);
    });

    test("expandSinglePhrase expands choices A|B", () => {
        const result = expandSinglePhrase("CREATE [TEMP|TEMPORARY] TABLE");
        assert.deepStrictEqual(result, ["CREATE TABLE", "CREATE TEMP TABLE", "CREATE TEMPORARY TABLE"]);
    });
    ...
    test("expandPhrases processes arrays", () => {
        ...
        assert.deepStrictEqual(result, ["CREATE TABLE", "CREATE OR REPLACE TABLE", "DROP TABLE"]);
    });

    test("expandSinglePhrase with deeply nested optional and choice", () => {
        const result = expandSinglePhrase("A [B [C|D] E]");
        assert.deepStrictEqual(result, ["A", "A B E", "A B C E", "A B D E"]);
    });
...
suite("config", () => {
    ...
    test("isTabularStyle returns true for tabularLeft", () => {
        assert.strictEqual(isTabularStyle({ indentStyle: "tabularLeft" } as unknown as FormatOptions), true);
    });

    test("isTabularStyle returns true for tabularRight", () => {
        assert.strictEqual(isTabularStyle({ indentStyle: "tabularRight" } as unknown as FormatOptions), true);
    });

    test("isTabularStyle returns false for standard", () => {
        assert.strictEqual(isTabularStyle({ indentStyle: "standard" } as unknown as FormatOptions), false);
    });
});
...
suite("tabularStyle", () => {
    ...
    test("isTabularToken returns true for RESERVED_CLAUSE", () => {
        assert.strictEqual(isTabularToken(TokenType.RESERVED_CLAUSE), true);
    });

    test("isTabularToken returns true for RESERVED_SELECT", () => {
        assert.strictEqual(isTabularToken(TokenType.RESERVED_SELECT), true);
    });

    test("isTabularToken returns true for RESERVED_SET_OPERATION", () => {
        assert.strictEqual(isTabularToken(TokenType.RESERVED_SET_OPERATION), true);
    });

    test("isTabularToken returns true for RESERVED_JOIN", () => {
        assert.strictEqual(isTabularToken(TokenType.RESERVED_JOIN), true);
    });

    test("isTabularToken returns true for LIMIT", () => {
        assert.strictEqual(isTabularToken(TokenType.LIMIT), true);
    });

    test("isTabularToken returns false for regular token types", () => {
        assert.strictEqual(isTabularToken(TokenType.OPEN_PAREN), false);
        assert.strictEqual(isTabularToken(TokenType.COMMA), false);
    });
