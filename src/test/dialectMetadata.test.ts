...
suite('AdapterFactory 方言元数据测试', () => {
    ...
    test('getAllMetadata 应包含 starrocks', () => {
        const all = AdapterFactory.getAllMetadata();
        assert.ok(all.some(m => m.dialect === 'starrocks'), '应包含 starrocks');
    });
    ...
    test('getAllMetadata 应包含 sqlserver', () => {
        const all = AdapterFactory.getAllMetadata();
        assert.ok(all.some(m => m.dialect === 'sqlserver'), '应包含 sqlserver');
    });
    ...
    test('getAllMetadata 应包含 oracle', () => {
        const all = AdapterFactory.getAllMetadata();
        assert.ok(all.some(m => m.dialect === 'oracle'), '应包含 oracle');
    });
    ...
    test('getAllMetadata 应包含 dameng', () => {
        const all = AdapterFactory.getAllMetadata();
        assert.ok(all.some(m => m.dialect === 'dameng'), '应包含 dameng');
    });
