...
suite('Node Transformers Test Suite', () => {
    ...
    suite('FunctionTransformer', () => {
        ...
        test('converts NOW() to CURRENT_TIMESTAMP (mysql to hive)', () => {
            const node = { type: 'function', name: { name: [{ type: 'default', value: 'NOW' }] }, args: { type: 'expr_list', value: [] }, over: null }
        ...
        test('converts IFNULL to COALESCE (mysql to hive)', () => {
            const node = { type: 'function', name: { name: [{ type: 'default', value: 'IFNULL' }] }, args: { type: 'expr_list', value: [] }, over: null }
            transformer.transform(node as never, null, null, makeCtx('mysql', 'hive'))
        ...
        test('converts CURRENT_TIMESTAMP to NOW (hive to mysql)', () => {
            const node = { type: 'function', name: { name: [{ type: 'default', value: 'CURRENT_TIMESTAMP' }] }, args: { type: 'expr_list', value: [] }, over: null }
