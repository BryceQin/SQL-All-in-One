...
suite('Dialect List Consistency', () => {
    test('dialectRegistry, allDialects, dialectData, and keywords/index agree on dialect set', () => {
        const registryDialects = new Set(
            getDialectEntries().map(e => e.sqlDialect)
        )
        // allDialects exposes 12 named exports; the keyword/data arrays exist for each.
        const allDialectsKeys = new Set([
            'hive', 'mysql', 'spark', 'flinksql', 'sql', 'postgresql',
            'bigquery', 'sqlite', 'starrocks', 'sqlserver', 'oracle', 'dameng',
        ])
        ...
        for (const d of registryDialects) {
            assert.ok(
                d in keywordMap,
                `dialectData.keywordMap missing dialect: ${d}`
            )
            assert.ok(
                d in functionSigMap,
                `dialectData.functionSigMap missing dialect: ${d}`
            )
            assert.ok(
                allDialectsKeys.has(d),
                `allDialects missing dialect: ${d}`
            )
        }
