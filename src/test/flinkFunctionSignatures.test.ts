...
suite("Flink Function Signatures Completeness Tests", () => {
    ...
    const requiredTableFunctions = ["TUMBLE", "HOP", "CUMULATE", "SESSION"];
    ...
    test("所有签名有非空描述", () => {
        for (const sig of functionSignatures) {
            assert.ok(sig.description.length > 0, `${sig.name} should have non-empty description`);
        }
    });

    test("所有签名有非空参数数组", () => {
        for (const sig of functionSignatures) {
            assert.ok(Array.isArray(sig.params), `${sig.name} should have params array`);
        }
    });

    test("所有签名有 category", () => {
        for (const sig of functionSignatures) {
            assert.ok(sig.category.length > 0, `${sig.name} should have category`);
        }
    });
