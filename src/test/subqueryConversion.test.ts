...
suite("MySQL <-> Hive Subquery Conversion Test Suite", () => {
    ...
    suite("Scalar subquery in SELECT list", () => {
        ...
        test("MySQL to Hive - multiple scalar subqueries in one SELECT", () => {
            const sql =
                "SELECT u.id, (SELECT COUNT(*) FROM orders WHERE user_id = u.id) AS cnt, (SELECT MAX(amount) FROM orders WHERE user_id = u.id) AS mx FROM users u";
            const result = converter.convert(sql, "mysql", "hive");
            const out = assertConverted(result);
    ...
    suite("Scalar subquery in HAVING / ORDER BY / CASE", () => {
        ...
        test("MySQL to Hive - scalar subquery inside CASE WHEN", () => {
            const sql =
                "SELECT id, CASE WHEN (SELECT COUNT(*) FROM orders WHERE user_id = u.id) > 0 THEN 'has' ELSE 'none' END AS flag FROM users u";
            const result = converter.convert(sql, "mysql", "hive");
            const out = assertConverted(result);
    ...
    suite("Nested and deep subqueries", () => {
        test("MySQL to Hive - nested scalar subquery (subquery within subquery)", () => {
            const sql =
                "SELECT id, (SELECT COUNT(*) FROM orders WHERE user_id IN (SELECT id FROM users WHERE status = 1)) AS cnt FROM users";
            const result = converter.convert(sql, "mysql", "hive");
            const out = assertConverted(result);
        ...
        test("MySQL to Hive - derived table in FROM with subquery", () => {
            const sql =
                "SELECT t.user_id, t.cnt FROM (SELECT user_id, COUNT(*) AS cnt FROM orders GROUP BY user_id) t WHERE t.cnt > (SELECT AVG(c) FROM (SELECT COUNT(*) AS c FROM orders GROUP BY user_id) x)";
            const result = converter.convert(sql, "mysql", "hive");
            const out = assertConverted(result);
    ...
    suite("CTE and UNION with subqueries", () => {
        test("MySQL to Hive - CTE with scalar subquery in main query", () => {
            const sql =
                "WITH active AS (SELECT id, name FROM users WHERE status = 1) SELECT id, (SELECT COUNT(*) FROM orders WHERE user_id = active.id) AS cnt FROM active";
            const result = converter.convert(sql, "mysql", "hive");
            const out = assertConverted(result);
        ...
        test("MySQL to Hive - UNION with scalar subquery on one side", () => {
            const sql =
                "SELECT id, (SELECT COUNT(*) FROM orders WHERE user_id = u.id) AS c FROM users u UNION SELECT id, 0 AS c FROM archived_users";
            const result = converter.convert(sql, "mysql", "hive");
            const out = assertConverted(result);
    ...
    suite("DML with subqueries", () => {
        ...
        test("MySQL to Hive - INSERT SELECT with scalar subquery", () => {
            const sql =
                "INSERT INTO report (user_id, order_count) SELECT id, (SELECT COUNT(*) FROM orders WHERE user_id = users.id) FROM users";
            const result = converter.convert(sql, "mysql", "hive");
            const out = assertConverted(result);
