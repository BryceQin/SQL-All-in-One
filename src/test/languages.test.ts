...
import {
    functions as postgresqlFunctions,
    functionSignatures as postgresqlFunctionSignatures,
} from "../dialects/postgresql/postgresql.functions";
...
import {
    functions as starrocksFunctions,
    functionSignatures as starrocksFunctionSignatures,
} from "../dialects/starrocks/starrocks.functions";
...
import {
    functions as sqlserverFunctions,
    functionSignatures as sqlserverFunctionSignatures,
} from "../dialects/sqlserver/sqlserver.functions";
...
function testDialectKeywords(suiteName: string, keywords: string[], dataTypes: string[], dialectSpecificKeywords: string[]): void {
    suite(suiteName, () => {
        ...
        test("contains dialect-specific keywords", () => {
            for (const kw of dialectSpecificKeywords) {
                assert.ok(keywords.includes(kw), `Expected dialect-specific keyword '${kw}' to be present in ${suiteName} keywords`);
            }
        });
    });
}
    ...
    suite(suiteName, () => {
        ...
        test("each function signature has name (string), params (string[]), description (string)", () => {
            for (const sig of functionSignatures) {
                ...
                assert.ok(Array.isArray(sig.params), `signature params should be an array for ${sig.name}`);
                ...
                assert.strictEqual(typeof sig.description, "string", `signature description should be string for ${sig.name}`);
            }
        });
        ...
        test("functionSignatures has no duplicate names", () => {
            ...
            assert.strictEqual(duplicates.length, 0, `Duplicate function signature names in ${suiteName}: ${duplicates.join(", ")}`);
        });

        test("contains common aggregate functions", () => {
            for (const fn of commonFunctionNames) {
                assert.ok(functions.includes(fn), `Expected function '${fn}' to be present in ${suiteName} functions`);
            }
        });
        ...
        test("function names present in functions are also present in functionSignatures for common functions", () => {
            for (const fn of commonFunctionNames) {
                ...
                assert.ok(signatureExists, `Expected function '${fn}' to have a signature in ${suiteName} functionSignatures`);
            }
        });
    });
}
...
suite("StarRocks Specific Functions", () => {
    ...
    test("functions contains StarRocks-specific functions", () => {
        for (const fn of starrocksSpecificFunctions) {
            assert.ok(starrocksFunctions.includes(fn), `Expected StarRocks-specific function '${fn}' to be present in starrocks functions`);
        }
    });
...
suite("SQL Server Specific Functions", () => {
    const sqlserverSpecificFunctions = ["GETDATE", "CONVERT", "TRY_CONVERT", "STRING_AGG", "IIF"];
...
suite("Oracle Specific Functions", () => {
    ...
    test("functions contains Oracle-specific functions", () => {
        for (const fn of oracleSpecificFunctions) {
            assert.ok(oracleFunctions.includes(fn), "Expected Oracle-specific function " + fn + " to be present in oracle functions");
        }
    });
...
suite("Dameng Specific Functions", () => {
    const damengSpecificFunctions = ["DM_HASH", "DM_ENCRYPT", "TO_DM_DATE"];

    test("functions contains Dameng-specific functions", () => {
        for (const fn of damengSpecificFunctions) {
            assert.ok(damengFunctions.includes(fn), "Expected Dameng-specific function " + fn + " to be present in dameng functions");
        }
    });
    ...
    test("functions contains Oracle-compatible functions retained in Dameng", () => {
        ...
        for (const fn of oracleCompatibleFunctions) {
            assert.ok(damengFunctions.includes(fn), "Expected Oracle-compatible function " + fn + " to be present in dameng functions");
        }
    });
...
suite("baseKeywords.ts", () => {
    ...
    test("each baseKeyword has keyword (string), syntax (string), description (string), category (string)", () => {
        for (const kw of baseKeywords) {
            assert.strictEqual(typeof kw.keyword, "string", `keyword should be string, got ${typeof kw.keyword}`);
    ...
    test("keywords have valid categories", () => {
        const validCategories = ["query", "join", "setop", "dml", "ddl", "window", "transaction", "auxiliary", "conditional", "type"];
        for (const kw of baseKeywords) {
            assert.ok(
    ...
    test("baseKeywords has no duplicate keywords", () => {
        ...
        assert.strictEqual(duplicates.length, 0, `Duplicate keywords in baseKeywords: ${duplicates.join(", ")}`);
    });
});
...
suite("keywords/index.ts", () => {
    ...
    test("getKeywordsForDialect results include base keywords like SELECT", () => {
        for (const dialect of ["hive", "mysql", "spark", "sql", "postgresql", "bigquery", "sqlite", "starrocks", "sqlserver"] as const) {
            ...
            assert.ok(keywordTexts.includes("SELECT"), `getKeywordsForDialect('${dialect}') should include SELECT`);
            assert.ok(keywordTexts.includes("FROM"), `getKeywordsForDialect('${dialect}') should include FROM`);
            assert.ok(keywordTexts.includes("WHERE"), `getKeywordsForDialect('${dialect}') should include WHERE`);
        }
    });
...
suite("Dialect Formatter Options", () => {
    function testDialectOptions(dialectName: string, options: DialectOptions): void {
        test(`${dialectName}.formatter has DialectOptions structure`, () => {
            assert.strictEqual(typeof options.name, "string", `${dialectName} options.name should be a string`);
        ...
        test(`${dialectName} reservedKeywords matches the dialect keywords array`, () => {
            ...
            assert.ok(to.reservedKeywords.length > 0, `${dialectName} reservedKeywords should be non-empty`);
        });
    }
