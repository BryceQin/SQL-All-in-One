...
suite("AstLinter Test Suite", () => {
    ...
    test("use_coalesce_over_isnull detects IFNULL when enabled", async () => {
        ...
        await vscode.workspace
            .getConfiguration("SQL-All-in-One")
            .update("lint.use_coalesce_over_isnull", { enabled: true }, vscode.ConfigurationTarget.Global);
        ...
        await vscode.workspace
            .getConfiguration("SQL-All-in-One")
            .update("lint.use_coalesce_over_isnull", undefined, vscode.ConfigurationTarget.Global);
        container.unregister(Tokens.RuleRegistry);
        container.registerSingleton(Tokens.RuleRegistry, createRuleRegistry);
    });
