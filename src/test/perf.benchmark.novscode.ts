...
function generateLongLineSql(lineCount: number): string {
    ...
    for (let i = 0; i < lineCount; i++) {
        lines.push(`SELECT column_${i} AS alias_${i}, COUNT(*) AS cnt_${i} FROM table_${i} GROUP BY column_${i} HAVING cnt_${i} > 10 ORDER BY cnt_${i} DESC`)
    }
    return lines.join('\n')
}
...
function computeLineColumnFastLocal(
    lineOffsets: number[],
    offset: number,
