...
suite("DataImporter - importFromJson", () => {
    ...
    test("should handle null values in JSON", async () => {
        ...
        fs.writeFileSync(filePath, JSON.stringify([{ id: 1, name: null, age: 30 }]));
...
suite("DataImporter - SQL value formatting", () => {
    test("should format NULL values correctly in generated SQL", async () => {
        ...
        const filePath = path.join(tempDir, "nulls.json");
        fs.writeFileSync(filePath, JSON.stringify([{ id: 1, name: null }]));
    ...
    test("should escape single quotes in string values", async () => {
        ...
        fs.writeFileSync(filePath, JSON.stringify([{ id: 1, name: "O'Brien" }]));
    ...
    test("should format numbers without quotes", async () => {
        ...
        fs.writeFileSync(filePath, JSON.stringify([{ id: 1, age: 30, score: 95.5 }]));
        ...
        const params = executedParams[0];
        assert.ok(
            params.some((p) => p.value === 30),
            "Should pass integer as a numeric parameter",
        );
        assert.ok(
            params.some((p) => p.value === 95.5),
            "Should pass decimal as a numeric parameter",
        );
