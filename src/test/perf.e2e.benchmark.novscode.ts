 ...
 * ones.
 */
function measureColdParse(engine: SqlParserEngine, sql: string, dialect: SqlDialect, iterations = 10): number {
    const times: number[] = [];
    for (let i = 0; i < iterations; i++) {
...

{
    ...
    for (const size of sizes) {
        ...
        assert.ok(
            firstNode && typeof firstNode === "object" && "type" in firstNode,
...

{
    ...
    for (const size of sizes) {
        ...
        assert.strictEqual(result.length, size, `splitSqlStatements should return ${size} statements, got ${result.length}`);
...

{
    ...
    for (const n of setSizes) {
        ...
        assert.strictEqual(prefixCache.size(), 0, `deleteByPrefix should remove all ${n} entries, ${prefixCache.size()} remain`);

        console.log(
            `[size=${n}] set: ${formatMs(setTime)} median | get: ${formatMs(getTime)} median | deleteByPrefix: ${formatMs(delTime)} median`,
        );
    }
}
...

{
    ...
    for (const size of sizes) {
        ...
        console.log(
            `[size=${size}] first format: ${formatMs(firstTime)} median | cached formatter: ${formatMs(cachedTime)} median | speedup: ${speedup(firstTime, cachedTime)}`,
        );
    }
