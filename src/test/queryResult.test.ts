...
suite("DataExporter - Pure Logic", () => {
    ...
    suite("CSV content generation", () => {
        ...
        test("should generate CSV without headers", () => {
            const rows: DataRow[] = [{ id: 1, name: "Alice", email: "alice@test.com" }];
        ...
        test("should handle NULL values as empty string", () => {
            const rows: DataRow[] = [{ id: 1, name: null, email: "test@test.com" }];
            const csv = generateCsv(rows, columns);
            const lines = csv.split("\n");
        ...
        test("should handle undefined values as empty string", () => {
            const rows: DataRow[] = [{ id: 1, name: undefined, email: "test@test.com" }];
            const csv = generateCsv(rows, columns);
            const lines = csv.split("\n");
            assert.strictEqual(lines[1], "1,,test@test.com");
        });
        ...
        test("should escape fields with commas", () => {
            const rows: DataRow[] = [{ id: 1, name: "Smith, Jr.", email: "test@test.com" }];
            const csv = generateCsv(rows, columns);
            const lines = csv.split("\n");
        ...
        test("should escape fields with newlines", () => {
            const rows: DataRow[] = [{ id: 1, name: "Line1\nLine2", email: "test@test.com" }];
            const csv = generateCsv(rows, columns);
        ...
        test("should escape fields with double quotes", () => {
            const rows: DataRow[] = [{ id: 1, name: 'Say "hello"', email: "test@test.com" }];
            const csv = generateCsv(rows, columns);
            const lines = csv.split("\n");
    ...
    suite("SQL INSERT content generation", () => {
        ...
        test("should handle batchSize larger than rows", () => {
            const rows: DataRow[] = [{ id: 1, name: "Alice", age: 30 }];
...
suite("QueryResultPanel - Serialization", () => {
    suite("_serializeResult logic", () => {
        function serializeResult(result: QueryResult, connectionName?: string, connectionColor?: string): Record<string, unknown> {
            return {
...
suite("QueryResult Types", () => {
    ...
    test("should create valid QueryResult with data", () => {
        const result: QueryResult = {
            queryId: "q-test-2",
            status: "success",
            columns: [{ name: "id", type: "INT", nullable: false, isPrimaryKey: true, isAutoIncrement: true, isEnum: false }],
            rows: [{ id: 1 }],
            rowCount: 1,
...
suite("Sort Logic", () => {
    function cycleSort(
        ...
        clickedCol: number,
    ): { col: number | null; dir: string | null } {
...
suite("DataExporter - Excel Placeholder", () => {
    test("should throw error for Excel export", () => {
        ...
        assert.rejects(() => exportToExcel(), /Excel export will be available in a future update/);
    });
});
...
suite("MonacoDataAdapter", () => {
    ...
    test("toMonacoDiagnostics converts VS Code diagnostics", () => {
        const diagnostics = [new vscode.Diagnostic(new vscode.Range(0, 0, 0, 6), "Avoid SELECT *", vscode.DiagnosticSeverity.Warning)];
...
suite("LanguageBridge", () => {
    ...
    test("hover returns null for unknown word", async () => {
        const bridge = createTestLanguageBridge(extensionUri);
        const result = await bridge.handleHoverRequest("SELECT xyzabc123 FROM users", { line: 0, column: 8 }, "mysql");
...
suite("LanguageBridge Integration", () => {
    ...
    test("full completion flow: exportLanguageData + handleCompletionRequest", async () => {
        ...
        const items = await bridge.handleCompletionRequest("SELECT * FROM ", { line: 0, column: 15 }, "mysql");
