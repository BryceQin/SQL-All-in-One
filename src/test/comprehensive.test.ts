...
} from "../formatter/nodeFormatters/CommonFormatter";
...
suite("SQL Formatter Core Tests", () => {
    suite("format() - basic formatting", () => {
        ...
        test("throws ConfigError for unsupported dialect", () => {
            assert.throws(() => format("SELECT 1", { language: "unsupported_dialect" as "sql" }), ConfigError);
        });

        test("throws error for non-string input", () => {
            assert.throws(() => format(123 as unknown as string), /无效的查询语句/);
        });
    ...
    suite("format() - CREATE TABLE variations", () => {
        ...
        test("formats CREATE TABLE with multiple columns and constraints", () => {
            const result = format(
                "CREATE TABLE orders (id INT PRIMARY KEY, user_id INT, amount DECIMAL(10,2), CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES users(id))",
                { language: "mysql" },
            );
    ...
    suite("format() - new formatting options", () => {
        ...
        test("respects newlineBeforeLateralView - parses valid Hive SQL", () => {
            try {
                const result = format("SELECT id FROM users LATERAL VIEW explode(tags) t AS tag", {
                    language: "hive",
...
suite("ValidateConfig Tests", () => {
    ...
    test("throws ConfigError for deprecated option multilineLists", () => {
        assert.throws(() => validateConfig({ multilineLists: 3 } as unknown as FormatOptions), ConfigError);
    });

    test("throws ConfigError for deprecated option newlineBeforeOpenParen", () => {
        assert.throws(() => validateConfig({ newlineBeforeOpenParen: true } as unknown as FormatOptions), ConfigError);
    });

    test("throws ConfigError for deprecated option newlineBeforeCloseParen", () => {
        assert.throws(() => validateConfig({ newlineBeforeCloseParen: true } as unknown as FormatOptions), ConfigError);
    });

    test("throws ConfigError for deprecated option aliasAs", () => {
        assert.throws(() => validateConfig({ aliasAs: "always" } as unknown as FormatOptions), ConfigError);
    });

    test("throws ConfigError for expressionWidth <= 0", () => {
        assert.throws(() => validateConfig({ expressionWidth: 0 } as unknown as FormatOptions), ConfigError);
    });

    test("throws ConfigError for negative expressionWidth", () => {
        assert.throws(() => validateConfig({ expressionWidth: -1 } as unknown as FormatOptions), ConfigError);
    });
...
suite("SqlParserEngine Tests", () => {
    ...
    test("throws ParseError for invalid SQL", () => {
        const engine = getParserEngine();
        assert.throws(() => engine.astify("NOT VALID SQL !!!", "mysql"), ParseError);
    });
...
suite("AstVisitor Tests", () => {
    ...
    test("walkAst visits all nodes", () => {
        const ast = {
            type: "select",
            from: [{ type: "table_ref", table: "users" }],
        };
...
suite("AstConverter Tests", () => {
    ...
    test("convertCreateTable throws when no CREATE TABLE found", () => {
        assert.throws(() => converter.convertCreateTable("SELECT * FROM users;", "mysql", "hive"), /No CREATE TABLE statement found/);
    });
...
suite("Format edge cases", () => {
    ...
    test("handles INSERT with ON DUPLICATE KEY UPDATE", () => {
        const result = format("INSERT INTO users (id, name) VALUES (1, 'test') ON DUPLICATE KEY UPDATE name = 'test'", {
            language: "mysql",
        });
