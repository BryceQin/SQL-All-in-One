...
suite('SparkSqlAdapter Tests', () => {
    ...
    suite('Delta Lake 语句 slot 化', () => {
        ...
        test('OPTIMIZE ... ZORDER BY (...) 整段 slot 化并还原', () => {
            ...
            assert.ok(!/\bOPTIMIZE\b/i.test(processedSql),
                'processed SQL should not contain OPTIMIZE')
        ...
        test('VACUUM ... RETAIN N HOURS 整段 slot 化', () => {
            ...
            assert.ok(!/\bVACUUM\b/i.test(processedSql),
                'processed SQL should not contain VACUUM')
            const restored = postprocessSparkSql(processedSql, state)
            assert.ok(/\bVACUUM\s+my_table\s+RETAIN\s+168\s+HOURS\b/i.test(restored),
                'restored should contain full VACUUM statement')
        })
    ...
    suite('原有功能回归', () => {
        ...
        test('LATERAL VIEW EXPLODE 仍正确处理', () => {
            ...
            const { processedSql, state } = preprocessSparkSql(sql)
            assert.ok(!/\bLATERAL\s+VIEW\b/i.test(processedSql),
                'processed SQL should not contain LATERAL VIEW')
        ...
        test('CREATE TABLE USING delta 仍正确处理', () => {
            ...
            assert.strictEqual(state.deltaStmtSlots.length, 0,
                'USING delta should not be slotted as delta statement')
            const restored = postprocessSparkSql(processedSql, state)
        ...
        test('SORT BY 仍正确还原', () => {
            ...
            assert.ok(state.keywordOccurrences.length > 0,
                'SORT BY should produce keyword occurrence')
        ...
        test('Delta 语句与普通 SELECT 共存', () => {
            ...
            const restored = postprocessSparkSql(processedSql, state)
            assert.ok(/\bOPTIMIZE\b[\s\S]*\bSELECT\b/i.test(restored),
                'restored should contain both OPTIMIZE and SELECT')
        })
...
DESCRIBE HISTORY t3;`
            const { processedSql, state } = preprocessSparkSql(sql)
            assert.strictEqual(state.deltaStmtSlots.length, 3,
                'should produce 3 delta slots')
            const restored = postprocessSparkSql(processedSql, state)
