...
suite("ConfigManager", () => {
    ...
    test("getSectionKeys() returns defaults for all keys", () => {
        const result = configManager.getSectionKeys<{ x: string; y: number }>("testPrefix", ["x", "y"], { x: "defaultX", y: 99 });
        assert.strictEqual(result.x, "defaultX");
        assert.strictEqual(result.y, 99);
    });

    test("getSectionKeys() caches result", () => {
        const first = configManager.getSectionKeys<{ k: string }>("testCache", ["k"], { k: "v1" });
        const second = configManager.getSectionKeys<{ k: string }>("testCache", ["k"], { k: "v2" });
...
suite("ErrorHandler", () => {
    ...
    test("try() returns function result on success", () => {
        const result = errorHandler.try(() => 42, "test-context");
    ...
    test("tryAsync() returns async function result on success", async () => {
        const result = await errorHandler.tryAsync(async () => "asyncResult", "async-context");
    ...
    test("tryAsync() rethrows async error when rethrow is true", async () => {
        ...
        try {
            await errorHandler.tryAsync(
                async () => {
                    throw new Error("async rethrow boom");
                },
                "async-context",
                { rethrow: true },
            );
    ...
    test("addListener() returns unsubscribe function", () => {
        const unsub = errorHandler.addListener(() => {
            /* no-op listener */
        });
...
suite("DIContainer", () => {
    ...
    test("get() throws for unregistered token", () => {
        assert.throws(() => container.get("NotRegistered"), /Service not registered: NotRegistered/);
    });
