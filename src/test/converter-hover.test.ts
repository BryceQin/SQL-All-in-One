...
import {
    MYSQL_TO_HIVE_FUNCTIONS,
    HIVE_TO_MYSQL_FUNCTIONS,
    convertIfnullToCoalesce,
    convertCoalesceToIfnull,
    MYSQL_TO_HIVE_FUNCTION_NAMES,
    HIVE_TO_MYSQL_FUNCTION_NAMES,
} from "../converter/functionMappings";
...
suite("Converter and Hover Module Tests", () => {
    ...
    suite("functionMappings - MYSQL_TO_HIVE_FUNCTIONS", () => {
        ...
        test("each MYSQL_TO_HIVE mapping has pattern (RegExp) and replacement (string)", () => {
            for (const mapping of MYSQL_TO_HIVE_FUNCTIONS) {
                assert.ok(mapping.pattern instanceof RegExp, `Pattern should be RegExp for replacement "${mapping.replacement}"`);
                assert.strictEqual(typeof mapping.replacement, "string", `Replacement should be string`);
            }
        });
    ...
    suite("hoverUtils", () => {
        ...
        suite("buildParameterMarkdown", () => {
            test("returns a vscode.MarkdownString", () => {
                const md = buildParameterMarkdown("param1", [{ line: 1, context: "${param1} = value" }]);
                assert.ok(md !== null && md !== undefined, "Should return a value");
                assert.ok(typeof md === "object", "Should return an object");
            });

            test("markdown includes parameter name", () => {
                const md = buildParameterMarkdown("myParam", [{ line: 1, context: "SELECT ${myParam}" }]);
                const value = (md as unknown as MarkdownStringInternal).value || "";
            ...
            test("markdown includes context strings", () => {
                const md = buildParameterMarkdown("param", [{ line: 3, context: "WHERE id = ${param}" }]);
                const value = (md as unknown as MarkdownStringInternal).value || "";
        ...
        suite("extractWordAtPosition", () => {
            test("is a callable function", () => {
                assert.strictEqual(typeof extractWordAtPosition, "function", "extractWordAtPosition should be a function");
            });

            test("is exported from hoverUtils", () => {
                assert.ok(extractWordAtPosition !== undefined, "extractWordAtPosition should be exported");
            });
        });
        ...
        suite("extractParameterAtPosition", () => {
            test("is a callable function", () => {
                assert.strictEqual(typeof extractParameterAtPosition, "function", "extractParameterAtPosition should be a function");
            });

            test("is exported from hoverUtils", () => {
                assert.ok(extractParameterAtPosition !== undefined, "extractParameterAtPosition should be exported");
            });
    ...
    suite("HoverResolver interface", () => {
        test("HoverResolver interface defines resolve method signature", () => {
            ...
            assert.strictEqual(typeof resolver.resolve, "function", "resolve should be a function");
            assert.strictEqual(
                resolver.resolve(
                    "test",
                    "hive",
