...
GROUP BY window_start`;
            ...
            assert.ok(!/\bTUMBLE\s*\(\s*TABLE\b/i.test(processedSql), "processed SQL should not contain TUMBLE(TABLE ...)");
            assert.ok(state.slots.length > 0, "should produce at least one slot");

            const restored = postprocessFlinkSql(processedSql, state);
            assert.ok(/\bTUMBLE\s*\(\s*TABLE\b/i.test(restored), "restored SQL should contain TUMBLE(TABLE ...)");
        });
    ...
    WATERMARK FOR ts AS ts - INTERVAL '5' SECOND
            ...
            const { processedSql, state } = preprocessFlinkSql(sql);
            assert.ok(!/\bWATERMARK\b/i.test(processedSql), "processed SQL should not contain WATERMARK keyword");
            const restored = postprocessFlinkSql(processedSql, state);
            assert.ok(/\bWATERMARK\s+FOR\s+ts\b/i.test(restored), "restored SQL should contain WATERMARK FOR ts");
        });
    ...
    'topic' = 'test'
            ...
            const { processedSql, state } = preprocessFlinkSql(sql);
            assert.ok(!/\bWITH\s*\(/i.test(processedSql), "processed SQL should not contain WITH (");
            const restored = postprocessFlinkSql(processedSql, state);
            assert.ok(/'connector'\s*=\s*'kafka'/.test(restored), "restored SQL should contain 'connector' = 'kafka'");
        });
    ...
    PRIMARY KEY (id) NOT ENFORCED
            ...
            const { processedSql, state } = preprocessFlinkSql(sql);
            assert.ok(!/\bPRIMARY\s+KEY\b/i.test(processedSql), "processed SQL should not contain PRIMARY KEY");
            const restored = postprocessFlinkSql(processedSql, state);
            assert.ok(
                /\bPRIMARY\s+KEY\s*\(id\)\s*NOT\s+ENFORCED\b/i.test(restored),
...
ON s.pid = p.id`;
            const { processedSql, state } = preprocessFlinkSql(sql);
            assert.ok(!/\bFOR\s+SYSTEM_TIME\b/i.test(processedSql), "processed SQL should not contain FOR SYSTEM_TIME");
            const restored = postprocessFlinkSql(processedSql, state);
            assert.ok(/\bFOR\s+SYSTEM_TIME\s+AS\s+OF\b/i.test(restored), "restored SQL should contain FOR SYSTEM_TIME AS OF");
        });
...
MATCH_RECOGNIZE (
    ...
    DEFINE
        ...
        DOWN AS DOWN.price < START_ROW.price
            ...
            const { processedSql, state } = preprocessFlinkSql(sql);
            assert.ok(!/\bMATCH_RECOGNIZE\b/i.test(processedSql), "processed SQL should not contain MATCH_RECOGNIZE");
            const restored = postprocessFlinkSql(processedSql, state);
            assert.ok(/\bMATCH_RECOGNIZE\b/i.test(restored), "restored SQL should contain MATCH_RECOGNIZE");
