...
import {
    ...
    SafetyConfirmation,
    QueryHistoryEntry,
} from "../database/query/QueryResult";
...
suite("SQL Execution Engine", () => {
    suite("SqlStatementDetector - detectStatementType", () => {
        ...
        test("should detect INSERT", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("INSERT INTO users VALUES (1)"),
                "INSERT",
            );
        });
        ...
        test("should detect UPDATE", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType('UPDATE users SET name = "a"'),
                "UPDATE",
            );
        });
        ...
        test("should detect CREATE", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("CREATE TABLE test (id INT)"),
                "CREATE",
            );
        });
        ...
        test("should detect ALTER", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("ALTER TABLE test ADD col INT"),
                "ALTER",
            );
        });
        ...
        test("should detect TRUNCATE", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("TRUNCATE TABLE test"),
                "TRUNCATE",
            );
        });
        ...
        test("should detect RENAME", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("RENAME TABLE old TO new"),
                "RENAME",
            );
        });
        ...
        test("should detect GRANT", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("GRANT SELECT ON db.* TO user"),
                "GRANT",
            );
        });
        ...
        test("should detect REVOKE", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("REVOKE SELECT ON db.* FROM user"),
                "REVOKE",
            );
        });
        ...
        test("should map WITH to SELECT (CTE)", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("WITH cte AS (SELECT 1) SELECT * FROM cte"),
                "SELECT",
            );
        });
        ...
        test("should handle SQL with leading whitespace", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("   SELECT * FROM users"),
                "SELECT",
            );
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("\n  INSERT INTO users VALUES (1)"),
                "INSERT",
            );
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).detectStatementType("\t\tDELETE FROM users"),
                "DELETE",
            );
        });
    ...
    suite("SqlStatementDetector - parseDelimiter", () => {
        ...
        test("should detect DELIMITER $$", () => {
            assert.strictEqual(
                (detector as unknown as SqlStatementDetectorInternal).parseDelimiter({
    ...
    suite("SafeQueryGuard - buildResult", () => {
        ...
        test("should mark unsafe when confirmations exist in moderate mode", () => {
            const result = (guard as unknown as SafeQueryGuardInternal).buildResult(
                "moderate",
                [],
                [{ rule: "drop_statement", message: "DROP", sql: "" }],
            );
            assert.strictEqual(result.safe, false);
        });
        ...
        test("should mark unsafe when warnings exist in strict mode", () => {
            const result = (guard as unknown as SafeQueryGuardInternal).buildResult(
                "strict",
                [{ rule: "delete_without_where", message: "DELETE", severity: "warning", sql: "" }],
                [],
            );
            assert.strictEqual(result.safe, false);
        });
    ...
    suite("MysqlAdapter - basic operations (no connection)", () => {
        ...
        test("should return errors for batch queries without connection", async () => {
            const results = await adapter.queryAdapter.executeBatch([{ sql: "SELECT 1" }, { sql: "SELECT 2" }]);
            assert.strictEqual(results.length, 2);
    ...
    suite("Integration - SafeQueryGuard with various SQL patterns", () => {
        ...
        test("should handle UPDATE with JOIN and WHERE", async () => {
            const result = await guard.analyze(
                'UPDATE users u JOIN orders o ON u.id = o.user_id SET u.status = "active" WHERE o.total > 100',
                "moderate",
            );
            assert.strictEqual(result.warnings.length, 0);
        });
