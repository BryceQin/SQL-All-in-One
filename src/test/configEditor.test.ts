...
suite("ConfigEditorPanel 测试", () => {
    suite("基础功能测试", () => {
        ...
        test("命令应该在命令面板中注册", async function () {
            ...
            assert.ok(commands.includes("hive-formatter.open-config-editor"), "Open Config Editor 命令未找到");

            assert.ok(commands.includes("hive-formatter.format-selection"), "Format Selection 命令未找到");

            assert.ok(commands.includes("hive-formatter.mysql-to-hive"), "MySQL to HiveSQL 命令未找到");

            assert.ok(commands.includes("hive-formatter.hive-to-mysql"), "HiveSQL to MySQL 命令未找到");
        });
    });
...
suite("回归测试 - 现有功能完整性", () => {
    suite("格式化功能回归测试", () => {
        test("基本 SQL 格式化应该正常工作", async function () {
            ...
            assert.ok(formattedText.includes("SELECT") || formattedText.includes("select"), "格式化后应包含 SELECT 关键字");
            assert.ok(formattedText.includes("FROM") || formattedText.includes("from"), "格式化后应包含 FROM 关键字");
        ...
        test("HiveQL 格式化应该正常工作", async function () {
            ...
            try {
                ...
                assert.ok(formattedText.includes("SELECT") || formattedText.includes("select"), "应包含 SELECT 关键字");
            } finally {
                if (originalDialect) {
    ...
    suite("完整工作流测试", () => {
        test("从配置到格式的端到端工作流", async function () {
            ...
            try {
                ...
                assert.ok(formattedText.toUpperCase().includes("SELECT"), "大写模式下应包含大写的 SELECT");
                assert.ok(formattedText.toUpperCase().includes("FROM"), "大写模式下应包含大写的 FROM");
                assert.ok(formattedText.toUpperCase().includes("WHERE"), "大写模式下应包含大写的 WHERE");
        ...
        test("多文件格式化一致性", async function () {
            ...
            const hasSimilarStructure =
                formatted1.toUpperCase().includes("SELECT") &&
                formatted2.toUpperCase().includes("SELECT") &&
                formatted1.toUpperCase().includes("FROM") &&
                formatted2.toUpperCase().includes("FROM");
