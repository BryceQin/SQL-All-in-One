...
class MockAstNavigator {
    ...
    getDefinition(_word: string, _index: SymbolIndex): vscode.Location | null {
        return null;
    }
    hasDefinition(_word: string, _index: SymbolIndex): boolean {
        return false;
    }
    detectSymbolType(_word: string, _index: SymbolIndex): SymbolType | null {
        return null;
    }
    findReferences(
...
function callValidateNewName(
    ...
    return (provider as unknown as { validateNewName(n: string, o: string, i: SymbolIndex, l: string): string | null }).validateNewName(
        newName,
        oldName,
        index,
        languageId,
...
suite("Navigation Module - Pure Logic Tests", () => {
    ...
    suite("invalidate", () => {
        test("invalidate clears document from cache", async () => {
            ...
            assert.strictEqual(result1 === null, result2 === null, "getAST results should be consistent before and after invalidate");
        });
