...
export function getCommentCompletionItems(doc: vscode.TextDocument, _pos: vscode.Position): vscode.CompletionItem[] {
    try {
...
function createHeaderItem(
    ...
    const inputTableLines =
        inputTables.length > 0 ? inputTables.map((tbl) => `--     - ${tbl}`).join("\n") : `--     ${t("completion.noTableDepsDetected")}`;

    const outputTableLines =
