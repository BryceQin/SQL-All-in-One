...
function getLocFromAny(item: unknown): LocRange | null {
    ...
    if (expr != null && typeof expr === "object") {
        ...
        if (
            exprLoc?.start?.line !== undefined &&
            exprLoc?.start?.column !== undefined &&
            exprLoc?.end?.line !== undefined &&
            exprLoc?.end?.column !== undefined
        ) {
...
function determineNodeContext(node: AstNode, parent: AstNode | null): CompletionContext {
    if (node.type === "select") {
        return "select_columns";
    }
    ...
    if (parent) {
        return determineNodeContext(parent, null);
    }

    return "unknown";
}

export function findCursorContext(
    sql: string,
    position: AstLocation,
    dialect: SqlDialect,
...
function findContextInStatement(node: AstNode, pos: AstLocation): CompletionContext {
    ...
    if (enclosing) {
        return determineNodeContext(enclosing.node, enclosing.parent);
    }
