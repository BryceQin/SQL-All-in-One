...
export const flinksqlKeywords: KeywordInfo[] = [
    {
        keyword: "WATERMARK",
        syntax: "WATERMARK FOR rowtime_column AS strategy",
        description: "定义表的水位线策略，用于事件时间处理",
        category: "hint",
        example: "CREATE TABLE t (\n  ts TIMESTAMP(3),\n  WATERMARK FOR ts AS ts - INTERVAL '5' SECOND\n)",
    },
    {
        keyword: "WITH",
        syntax: "CREATE TABLE ... WITH (key=value, ...)",
        description: "定义表的连接器属性",
        category: "hint",
        example: "CREATE TABLE t (\n  id INT\n) WITH (\n  'connector' = 'kafka'\n)",
    },
    {
        keyword: "TUMBLE",
        syntax: "TUMBLE(TABLE data, DESCRIPTOR(timecol), size)",
        description: "滚动窗口表值函数",
        category: "hint",
        example:
    ...
    {
        keyword: "CUMULATE",
        syntax: "CUMULATE(TABLE data, DESCRIPTOR(timecol), step, size)",
        description: "累积窗口表值函数",
        category: "hint",
    },
