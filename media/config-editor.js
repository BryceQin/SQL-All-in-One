...
function applyI18nDict(dict) {
    ...
    window.applyI18n(document, function (key) {
        return dict[key];
    });
}
...
function clearAllModifiedDots() {
    document.querySelectorAll(".ci-modified-dot").forEach(function (dot) {
        dot.remove();
    });
}
...
function saveGroupState(group, isOpen) {
    ...
    if (key) {
        try {
            localStorage.setItem(GROUP_STATE_PREFIX + key, isOpen ? "1" : "0");
        } catch (e) {}
    }
}
...
function loadGroupState(group) {
    ...
    if (!key) return null;
    try {
        return localStorage.getItem(GROUP_STATE_PREFIX + key);
    } catch (e) {
        return null;
    }
}
...
function renderConnections(connections, groups) {
    ...
    connections.forEach(function (conn) {
        ...
        var stateClass =
            state === "connected" ? "conn-state-connected" : state === "failed" ? "conn-state-failed" : "conn-state-disconnected";
        var testTime = connectionTestTimes[conn.id];
        var testTimeText = testTime
            ? (getI18nDict()["configEditor.conn.recentlyTested"] || "{0}前测试").replace("{0}", formatRelativeTime(testTime))
            : "";
        ...
        html +=
            '<div class="conn-item" data-conn-id="' +
            conn.id +
            '">' +
            '<div class="conn-item-color" style="' +
            colorStyle +
            '"></div>' +
            '<div class="conn-item-state ' +
            stateClass +
            '" title="' +
            state +
            '"></div>' +
            ...
            "</div>" +
            '<div class="conn-item-detail">' +
            detail +
            "</div>" +
            "</div>" +
            '<div class="conn-item-actions">' +
            '<button class="btn btn-secondary btn-sm" data-action="editConnection" data-action-arg="' +
            conn.id +
            '">✏️</button>' +
            '<button class="btn btn-secondary btn-sm" data-action="testConnection" data-action-arg="' +
            conn.id +
            '">🔌</button>' +
            '<button class="btn btn-secondary btn-sm" data-action="deleteConnection" data-action-arg="' +
            conn.id +
            '">🗑️</button>' +
            "</div></div>";
    });
...
function renderCfDialect() {
    ...
    if (showMoreDialects) {
        knownDialects.forEach(function (key) {
            var isSupported = supportedDialects.some(function (m) {
                return m.dialect === key;
            });
            if (!isSupported) {
                var opt = document.createElement("option");
                opt.value = key;
                opt.textContent =
                    (DIALECT_DISPLAY_NAMES[key] || key) + " (" + (getI18nDict()["configEditor.conn.unsupportedDialect"] || "不支持") + ")";
                opt.disabled = true;
    ...
    if (
        currentValue &&
        supportedDialects.some(function (m) {
            return m.dialect === currentValue;
        })
    ) {
...
function collectConnFormData() {
    ...
    var data = {
        ...
        database:
            dialect === "sqlite"
                ? document.getElementById("cfSqliteFile").value.trim()
                : document.getElementById("cfDatabase").value.trim() || undefined,
...
function validateConnForm() {
    ...
    if (dialect !== "sqlite") {
        if (!document.getElementById("cfHost").value.trim())
            errors.push(getI18nDict()["configEditor.conn.error.hostRequired"] || "主机地址不能为空");
        ...
        if (!document.getElementById("cfUsername").value.trim())
            errors.push(getI18nDict()["configEditor.conn.error.userRequired"] || "用户名不能为空");
    } else {
        if (!document.getElementById("cfSqliteFile").value.trim())
            errors.push(getI18nDict()["configEditor.conn.error.sqliteRequired"] || "SQLite 文件路径不能为空");
    }

    if (document.getElementById("cfSshEnabled").checked) {
        if (!document.getElementById("cfSshHost").value.trim())
            errors.push(getI18nDict()["configEditor.conn.error.sshHostRequired"] || "SSH 主机不能为空");
        if (!document.getElementById("cfSshUsername").value.trim())
            errors.push(getI18nDict()["configEditor.conn.error.sshUserRequired"] || "SSH 用户名不能为空");
    }
...
function handleEditConnectionDetail(message) {
    ...
    document.getElementById("connFormTitle").textContent =
        (getI18nDict()["configEditor.conn.editConnection"] || "编辑连接") + " - " + message.connection.name;
    showConnForm(true);
}
